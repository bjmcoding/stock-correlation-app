...
def fetch_max_stock_data(tickers_tuple):
    """
    Fetches up to MAX_HISTORY_YEARS of data for tickers using yahooquery.
    Returns: DataFrame, list of fetched tickers, list of log messages.
    """
    tickers = list(tickers_tuple)
    log_messages = []
    if not tickers:
        return None, [], ["Error: No tickers provided."]

    log_messages.append(f"INFO: Fetching max history ({MAX_HISTORY_YEARS} years) for {len(tickers)} tickers with yahooquery...")
    fetch_start_date = MAX_FETCH_START_DATE
    fetch_end_date = MAX_FETCH_END_DATE
    log_messages.append(f"DEBUG: fetch_start_date={fetch_start_date}, fetch_end_date={fetch_end_date}")

    try:
        ticker_obj = Ticker(tickers, asynchronous=True, validate=True)
        raw_data = ticker_obj.history(start=fetch_start_date, end=fetch_end_date, interval='1d')

        if not isinstance(raw_data, pd.DataFrame) or raw_data.empty:
            log_messages.append("ERROR: yahooquery returned no data.")
            return None, [], log_messages

        # Extract 'adjclose' and pivot
        adj_close_data = raw_data.reset_index()[['symbol', 'date', 'adjclose']]
        processed_data = adj_close_data.pivot(index='date', columns='symbol', values='adjclose')

        # Post-processing
        processed_data = processed_data.sort_index().ffill().bfill()
        processed_data = processed_data.dropna(axis=1, how='all')
        final_tickers_list = list(processed_data.columns)

        if not final_tickers_list:
            log_messages.append("ERROR: Dataframe became empty after processing.")
            return None, [], log_messages

        log_messages.append(f"INFO: Final data contains {len(final_tickers_list)} tickers out of {len(tickers)} requested.")
        return processed_data, final_tickers_list, log_messages

    except Exception as e:
        log_messages.append(f"ERROR: An error occurred with yahooquery: {str(e)}")
...
with col1:
    ...
    # --- Refresh Data Cache ---
    if st.button("Clear Data Cache & Force Refresh"):
        st.cache_data.clear()
        st.success("Cache cleared! Re-run the analysis to fetch the latest data.")
        st.stop()
    # --- END LOG DISPLAY ---
